...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.profile-edit-form input, .profile-edit-form select').forEach(el => {
    ...
    // Add special classes for selects
    if (el.tagName === 'SELECT') {
      ...
      // Style options on hover (for multiple selects)
      if (el.multiple) {
  ...
  document.querySelectorAll('[data-bs-toggle="collapse"]').forEach(btn => {
    ...
    // Add necessary classes for smooth transitions
  ...
  function setupAllergenCheckboxes() {
    ...
    // Get all original input elements
    const originalInputs = [...allergensContainer.querySelectorAll('input[type="checkbox"]')];
    
    // Create UL container
    const ul = document.createElement('ul');
    
    // For each input, create appropriate list structure
    originalInputs.forEach(input => {
      // Get text from existing label or nearby element
      ...
      // Clone input to preserve any attributes and state
    ...
    // Save helper text if it exists
    const helperText = allergensContainer.querySelector('.allergen-helper');
    
    // Empty container and add new structure
    ...
    if (helperText) {
      ...
      // Create new helper text if it didn't exist
    ...
    // Re-apply event listeners to new checkboxes
  ...
  function attachAllergensEventListeners() {
    ...
    if (allergensCheckboxes.length > 0) {
      allergensCheckboxes.forEach(checkbox => {
        // Remove any existing listeners (for safety)
